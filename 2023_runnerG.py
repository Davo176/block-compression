...
parser = argparse.ArgumentParser(
    ...
    epilog=
        ...
        "Compression is measured as the a ratio V:A, where V is the volume\n"
        "in block units of the model (sum of the volume of all of the blocks)\n"
        "and A is total surface area of all of the blocks (sum of the surface\n"
        "areas of all blocks). For a block model with no compression, V:A ratio\n"
        "is 1:6. Any aggregation of blocks will result in a higher number, the\n"
        "maximum limit being data dependent.\n"
        "\n"
...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    total_volume = x_count * y_count * z_count
    total_area = 0
    ...
    if block_count_f1 < 50000000:
                    ...
                    for sz in range(zs2):
                        for sy in range(ys2):
                            for sx in range(xs2):
                                ...
                                idx += 1
                    # Account for the area in the area total
                    total_area += 2*(xs2*ys2) + 2*(xs2*zs2) + 2*(ys2*zs2)
        ...
        print("Error: checking of files > 50000000 blocks is NYI.", file=stderr)
        sys.exit(1)
    toc = timeit.default_timer()
...
print("-----------", file=stdvout)
compression = total_volume / total_area
print("Compression = {:6.2f}%  (surface area reduced from {} to {})".format(
    compression*100,
    total_volume * 6,
    total_area), file=stdvout)
print("-----------", file=stdvout)
