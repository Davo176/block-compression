...
inline void copy_buffer(const int &start_x, const int &start_y, const int &start_z, const int &px, const int &py, const int &pz, char ***cube_buffer, char ***dest_buffer)
{
...
inline void xyzRleCompress0(const int start_x, const int start_y, const int start_z, const int &px, const int &py, const int &pz, const int &ox, const int &oy, const int &oz, char ***c_buffer, std::unordered_map<char, std::string> &tags)
{
    /*
    Function Name: xyzRleCompress0
    Function Arguments:
    ...
    */
    ...
    int xx = 3, yy = 3, zz = 3;
    current = c_buffer[z][y][x];

    if (current != '$' && current != '\0' && x > 0 && y > 0)
    {
        // std::cout<<"from xyz rle"<<x<<" "<<y<<" "<<z<<std::endl;
        ...
        while (true)
        {
            ...
            for (int k = 0; k < zz; ++k)
            {
                for (int i = 0; i < yy; ++i)
                {
                    for (int j = 0; j < xx; ++j)
                    {
                        if (current != c_buffer[z + k][y + i][x + j])
                        {
                            value = 0;
                            break;
                        }
            ...
            if (value == 0 && xx > 3)
            {
                ...
                y++;
                z++;
                std::cout << x << "," << y << "," << z << "," << xx - 2 << "," << yy - 2 << "," << zz - 2 << "," << tags[current] << "\n";
                for (int k = 0; k < zz - 2; ++k)
                {
                    for (int i = 0; i < yy - 2; ++i)
                    {
                        for (int j = 0; j < xx - 2; ++j)
                        {
                            c_buffer[z + k][y + i][x + j] = '$';
                        }
                    }
                }
                break;
            ...
            else if (value == 1)
            {
                ...
                yy += 2;
                zz += 2;
                if (xx >= px || yy >= py || zz >= pz || x == 0 || y == 0 || z == 0)
                {
                    if (xx - 2 > 3)
                    {
                        std::cout << x << "," << y << "," << z << "," << xx - 2 << "," << yy - 2 << "," << zz - 2 << "," << tags[current] << "\n";
...
Node *buildOctree(const int &px, const int &py, const int &pz, char ***cube_buffer,
                  const int &ox, const int &oy, const int &oz, std::unordered_map<char, std::string> &tags)
{
    /*
    ...
    */
    ...
    Node *node = new Node();
    // Termination conditon for the recursion
    if (px <= 1 && py <= 1 && pz <= 1)
    {
    ...
    if (!node->isLeaf)
    {
        // if(hx==128 && hy==128 && hz==128){
        if (ox - px > 4 && oy - py > 4 && oz - pz > 4)
        {
            xyzRleCompress0(hx, hy, hz, px, py, pz, ox, oy, oz, cube_buffer, tags);
        }

        node->children[0] = buildOctree(hx, hy, hz, cube_buffer, ox, oy, oz, tags);
        node->children[1] = buildOctree(px - hx, hy, hz, cube_buffer, ox + hx, oy, oz, tags);
        node->children[2] = buildOctree(hx, py - hy, hz, cube_buffer, ox, oy + hy, oz, tags);
        node->children[3] = buildOctree(px - hx, py - hy, hz, cube_buffer, ox + hx, oy + hy, oz, tags);
        if (pz > 1)
        {
            node->children[4] = buildOctree(hx, hy, pz - hz, cube_buffer, ox, oy, oz + hz, tags);
            node->children[5] = buildOctree(px - hx, hy, pz - hz, cube_buffer, ox + hx, oy, oz + hz, tags);
            node->children[6] = buildOctree(hx, py - hy, pz - hz, cube_buffer, ox, oy + hy, oz + hz, tags);
            node->children[7] = buildOctree(px - hx, py - hy, pz - hz, cube_buffer, ox + hx, oy + hy, oz + hz, tags);
        }
    }
    else
    {
...
void encodeAndPrintBlocks(Node *node, const int &ox, const int &oy, const int &oz,
                          const int &size_x, const int &size_y, const int &size_z, std::unordered_map<char, std::string> &tags, char ***cube_buffer)
{
    /*
   ...
   */
    if (node->isLeaf)
    {
        ...
        if (node->block_value.value != '$' && node->block_value.value != '\0')
        {
            if (size_x > 1)
            {
    ...
    else
    {
        ...
        encodeAndPrintBlocks(node->children[0], ox, oy, oz, hx, hy, hz, tags, cube_buffer);
        encodeAndPrintBlocks(node->children[1], ox + hx, oy, oz, size_x - hx, hy, hz, tags, cube_buffer);
        encodeAndPrintBlocks(node->children[2], ox, oy + hy, oz, hx, size_y - hy, hz, tags, cube_buffer);
        encodeAndPrintBlocks(node->children[3], ox + hx, oy + hy, oz, size_x - hx, size_y - hy, hz, tags, cube_buffer);
        if (size_z > 1)
        {
            encodeAndPrintBlocks(node->children[4], ox, oy, oz + hz, hx, hy, size_z - hz, tags, cube_buffer);
            encodeAndPrintBlocks(node->children[5], ox + hx, oy, oz + hz, size_x - hx, hy, size_z - hz, tags, cube_buffer);
            encodeAndPrintBlocks(node->children[6], ox, oy + hy, oz + hz, hx, size_y - hy, size_z - hz, tags, cube_buffer);
            encodeAndPrintBlocks(node->children[7], ox + hx, oy + hy, oz + hz, size_x - hx, size_y - hy, size_z - hz, tags, cube_buffer);
        }
    }
}

inline void xyzRleCompress(const int &start_x, const int &start_y, const int &start_z, const int &px, const int &py, const int &pz, const int &ox, const int &oy, const int &oz, char ***c_buffer, std::unordered_map<char, std::string> &tags, int print = 0)
{
    /*
...
*/
    ...
    copy_buffer(start_x, start_y, start_z, px, py, pz, c_buffer, cube_buffer);
    ...
    for (int z = start_x; z < pz; ++z)
    {
        for (int y = start_y; y < py; ++y)
        {
            for (int x = start_z; x < px; ++x)
            {
                if (cube_buffer[z][y][x] != '\0' && cube_buffer[z][y][x] != '$')
                {
...
int main()
{
    ...
    Node *root = buildOctree(px, py, pz, cube_buffer, 0, 0, 0, tags);
    encodeAndPrintBlocks(root, ox, oy, oz, px, py, pz, tags, cube_buffer);
    xyzRleCompress(0, 0, 0, px, py, pz, ox, oy, oz, cube_buffer, tags, 1);
