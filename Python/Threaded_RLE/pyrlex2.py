...
if __name__ == '__main__':
    ...
    # Select number of threads to run
    no_threads = 4
    threads = []

    # Create list of all threads
    for i in range(no_threads):
        threads.append(threading.Thread(target=process_lines, args=(lines, no_lines, i, no_threads, tag_table)))

    # For each thread in list start thread
    for i in range(no_threads):
        threads[i].start()
